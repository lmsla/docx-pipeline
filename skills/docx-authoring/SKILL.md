...
因此**合規的責任在產出當下**：交付前必須完成第 7 步的敏感資訊自檢，
不能倚賴下游有沒有人跑驗證。「合規」不等於「一律去識別化」——是否需要去識別化
取決於文件去向，兩級規則見第 7 步；憑證類一律處理，識別類則由使用者決定。

## 工作流程

1. 確認文件範圍。若本次對話涵蓋多個不相關主題，且使用者的請求沒有指定範圍
   （例如只說「整理成筆記」，未指明是哪一段討論），**停下來詢問要整理哪個
   主題，或是否要拆成多份文件**——不要自行判斷把不相關的討論合併進同一份
   文件，那會產出主題失焦、日後難以查找的文件。使用者已明確指定範圍時
   （例如「把剛剛的 snapshot 問題整理成筆記」）直接採用，不需詢問。

2. 確認文件類型，不得依語境自行推測，只選一種：
...
3. 讀取與本檔同一層的 `templates/ai-agent-markdown-rules.md`。
4. 再讀取選定的模板：
   - `templates/engineering-note-template.md`
   - `templates/enterprise-sop-template.md`
   這三個檔案都放在 SKILL.md 隔壁的 `templates/` 目錄下，四種安裝方式（Claude Code
   plugin、Antigravity、Codex、claude.ai 帳號層級 Skill）的相對位置一致，不需要
   平台專屬的 root 變數。若找不到隨附模板，停止整理並回報，不要自行重建規則。
5. 確認 `author`（撰寫者姓名），這是文件的追溯依據，必須在動筆前取得。依序判斷：
...
6. 依模板整理內容，完整保留使用者提供的技術細節、指令、輸出、圖片與證據；不得把推測寫成已驗證結果。
