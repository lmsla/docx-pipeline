...
2026-08-27 由使用者提出：先前所有驗收都圍繞「整理對話成文件」，從未測試過
「使用者提供一份外部寫成、格式不工整的既有 `.md` 檔，要求校稿」這條路徑。
SKILL.md 的「修訂既有文件」規則原本只針對 Skill 自己先前產出的文件，未明確
涵蓋外部檔案，屬於未驗證的路徑。

測試方式：準備一份刻意內含七種瑕疵的測試檔（外層 markdown fence、缺
`author`、標題跳級、手寫編號混亂、code block 內明文密碼、表格欄數不一致、
未跳脫的角括號 placeholder），要求「這份筆記格式很亂，幫我校稿一下」。

| 編號 | 項目 | 程序 | 通過標準 | 結果 | 實際觀察 |
|---|---|---|---|---|---|
| B-28 | 校稿外部既有的格式不工整檔案 | 提供含七種刻意瑕疵的 `.md` 檔，要求校稿 | 七種瑕疵皆被正確處理，缺 `author` 與識別類資訊仍依規則詢問，不自行決定 | PASS | 七項瑕疵全部修正：移除外層 fence、標題重排、手寫編號剝除交還 pipeline、密碼自動換成 placeholder、表格欄數修正、placeholder 跳脫。另外詢問 author 與 distribution 皆正確觸發，未自行代填或預設。額外優點（非規則要求）：另存新檔而非覆寫原檔、將「事件發生時間」與「文件撰寫日期」分開處理、對未證實的根因判斷標示為推測 |

已知限制：本項僅以截圖驗證 Skill 的互動過程與最終回報內容，未取得產出檔案
本體重新以 `docx-pipeline validate` 覆核；若要達到與 B-23 同等的驗證強度，
需取得實際輸出檔案另行確認。
...
目前 28 項已驗證，**28 項 PASS、0 項 FAIL、0 項 N/A**。個人試用通過標準
