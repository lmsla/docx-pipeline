...
| 編號 | 項目 | 程序 | 通過標準 | 結果 | 實際觀察 |
|---|---|---|---|---|---|
| B-04 | 語境清楚仍應詢問（設計已變更） | 討論一段部署與驗證流程後要求整理，未明講類型 | 停下來詢問是筆記還是 SOP | SUPERSEDED | 2026-08-24 依舊判準（語境清楚可直接判斷）測得 PASS；當日因 B-06 發現語境判斷可能失準，改為「除非使用者明講類型，否則一律詢問」，原判準已作廢，待依新規則重測 |
| B-05 | 語境清楚仍應詢問（設計已變更） | 討論一次問題調查後要求整理，未明講類型 | 停下來詢問是筆記還是 SOP | SUPERSEDED | 同 B-04：舊判準下 PASS，設計變更後原判準作廢，待重測 |
| B-06 | 未明講類型時應詢問（不限模糊情境） | 描述一次已解決的單一事件，未明講類型，要求整理 | 停下來詢問是筆記還是 SOP | FAIL→已修正設計 | 2026-08-24 實測：未詢問，直接判定為 Engineering Note。內容本身讀起來確為筆記而非 SOP，判斷結果合理，但過程未經確認——暴露的是「語境推測」這個機制本身的風險，而非這次判斷錯誤。已改為 SKILL.md 規則：除非使用者明講類型，否則一律詢問，不依語境判斷。待依新規則重測 |
...
目前 20 項已驗證，**16 項 PASS、0 項 FAIL、1 項 N/A、3 項 SUPERSEDED 待重測**
（B-04、B-05、B-06）。個人試用通過標準（B-01、B-07、B-12、B-19）**四項全數通過**，
不受此次設計變更影響（author 與敏感資訊自檢的判準未變）。

團隊推廣通過標準仍缺 B-04、B-05、B-06（待依新規則重測）、B-17、B-18，另有 B-03
待改用 zip 封裝後才能重新驗證。**個人日常試用已可正式使用**；團隊推廣待補上述項目。
重啟後的測試一律使用全新 session，不可恢復分頁。
