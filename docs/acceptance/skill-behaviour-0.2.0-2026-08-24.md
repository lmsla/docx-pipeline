...
2026-08-25 依序重測 B-06、B-05、B-04、B-17，四項皆 PASS，確認文件類型
「一律先問」的設計變更有效。B-18 測試時發現**新的真實缺陷**：無 `distribution`
的新文件遇到識別類資訊時，未詢問就自行預設為 `internal`，違反既有規則——
規則本身沒錯，是措辭強度不足（描述句而非強制句），且緊鄰「內部技術筆記通常
原樣保留」的說明，容易被讀成可以逕自假設為內部。已改為粗體強制句並明文禁止
預設值（v0.2.6），B-18 待重測。

目前 26 項已驗證，**25 項 PASS、1 項 FAIL（待重測）、1 項 N/A**。個人試用通過
標準（B-01、B-07、B-12、B-19）四項全數通過。

團隊推廣通過標準仍缺 B-18（待依 v0.2.6 重測），另有 B-03 待改用 zip 封裝後才能
重新驗證。**個人日常試用已可正式使用，但 distribution 的預設行為在 v0.2.6 前
有實際缺陷，已修正、待重測確認**。重啟後的測試一律使用全新 session，不可恢復
分頁。
