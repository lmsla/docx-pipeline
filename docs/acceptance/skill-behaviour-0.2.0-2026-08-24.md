...
v0.2.8 新增規則：對話涵蓋多個不相關主題、且使用者未指定範圍時，須先詢問要整理
哪個主題或是否拆成多份，不得自行合併。此規則由使用者提出的假設情境直接促成，
SKILL.md 先前完全未涵蓋，屬於本次驗收新發現、當場修正、當場驗證的項目。

| 編號 | 項目 | 程序 | 通過標準 | 結果 | 實際觀察 |
|---|---|---|---|---|---|
| B-27 | 多主題對話應詢問範圍 | 同一對話依序討論兩個不相關主題（ES 磁碟空間、S3 snapshot IAM 權限），僅說「幫我整理成筆記」，不指定範圍 | 停下來詢問要整理哪個主題或是否拆成多份，不自行合併 | PASS | 2026-08-26 實測：正確識別出對話涵蓋四個子主題（不只兩個大類，連 ILM 與 template 關聯排查都拆成獨立子題），用複選讓使用者勾選範圍，並主動建議三種拆分方案（拆兩份／合一份／拆四份）供選擇，超出規則字面要求的「問要哪個主題」。最終依使用者選擇的方案產出兩份主題各自獨立的檔案，未合併 |
...
2026-08-26 使用者提出假設情境（同一對話涵蓋多個不相關主題時會不會被混寫成
一份文件），查證後確認 SKILL.md 完全未涵蓋，當場新增規則（v0.2.8）並實測
B-27，PASS。

目前 27 項已驗證，**27 項 PASS、0 項 FAIL、0 項 N/A**。個人試用通過標準
（B-01、B-07、B-12、B-19）與團隊推廣通過標準（另加 B-04 至 B-06、B-14 至
B-17、B-22、B-23、B-10、B-18、B-03、B-27）**全數通過**。
