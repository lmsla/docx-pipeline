...
| B-05 | 停下來詢問文件類型，未明講時不自行判斷 | 未經詢問直接選定任一類型 |
...
**觀察重點**（對應 B-04，v0.2.3 判準已變更）

- 素材未明講「SOP」二字，即使語境已經很清楚（讓值班人員照著做、標準流程），
  仍應停下來詢問是要 Engineering Note 還是 Enterprise SOP，不依語境自行判斷。
- 若直接判定為 SOP、未經詢問，判定為 FAIL——即使判斷結果正確，跳過詢問本身
  就是違反規則。
...
- 不應自行選定並直接產出，即使內容讀起來明顯偏向某一類型也一樣——
  v0.2.3 起文件類型一律詢問，不依語境判斷是這條規則要防的正是這種情況。
